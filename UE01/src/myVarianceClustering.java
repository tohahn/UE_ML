...
public class myVarianceClustering {
    //uses the builtin distance function
    ...
    public Map<Point2D, List<Point2D>> cluster(List<Point2D> instances, int number) {
        ...
        //while errors get smaller, do clustering
        while (oldError != newError) {
            ...
            //does the clustering, finding new centroid, if different change data structure for next iteration
    ...
    private List<Point2D> getInitialCentroids(List<Point2D> instances, int number) {
        //this function assigns the points randomly to clusters and calculates the first cnetroids based on it
